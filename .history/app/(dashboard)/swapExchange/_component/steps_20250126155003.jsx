...
export function AmountSelectionStep() {
  ...
  const handleAmountChange = (value) => {
    ...
    if (numericValue > 0 && selectedCurrency) {
      ...
      const tradingFee = result?.total * 0.0005; // 0.05% trading fee
      setValue("localAmount", (result?.total - tradingFee).toString() || "");
    }
  };
        ...
        <Controller
          ...
          render={({ field }) => (
            <div>
              ...
              <p className="text-gray-500 text-sm mt-1">Trading fee (0.05%)</p>
            </div>
          )}
        />
