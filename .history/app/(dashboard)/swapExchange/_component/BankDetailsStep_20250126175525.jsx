...
export function BankDetailsStep() {
  ...
  const handleAccountNumberBlur = async () => {
    if (bankId && accountNumber.length === 10) {
      try {
        const accountData = await apiService.attributes.resolveAccount(bankId, accountNumber);
    ...
    <div>
      ...
      <div className="mb-6 p-4 bg-gray-100 rounded-md">
        <p className="text-sm text-gray-700">
          <strong>Total amount payable in Espees:</strong> 10.5 Espee(s)
        </p>
        <p className="text-sm text-gray-700">
          <strong>Amount to receive in Naira:</strong> ₦16500.0
        </p>
      </div>
