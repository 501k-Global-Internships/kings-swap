...
const Step1Form = ({
  ...
  const handleContinue = () => {
    ...
    // If countries aren't loaded yet, use the initial country ID
    onNext({ country_id: selectedCountry?.id || initialCountry });
  };
  ...
  return (
    <div className="w-full max-w-md p-4 bg-white rounded shadow-md">
      ...
      <div className="relative mb-4">
        ...
        {isOpen && isLoading && (
          <div className="absolute z-10 w-full bg-white border border-gray-300 rounded-b mt-[-1px] max-h-48 overflow-y-auto flex items-center justify-center p-4">
            <Loader2 className="w-6 h-6 animate-spin text-blue-500" />
          </div>
        )}
      </div>
      ...
      >
        {isLoading && countries.length === 0 ? "Loading..." : "Continue"}
      </button>
    </div>
  );
};
